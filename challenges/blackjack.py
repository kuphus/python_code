...
class Blackjack():
    ...
    def __init__(player_amount, player_number, visibility_rules, number_of_decks, *exclude_cards):
        deck[] = create_deck(number_of_decks, *exclude_cards)
        self.visibility_rules = "FIRST" # visibility rules: ALL, FIRST, SECOND, NONE
        self.players = []
    ...
    def get_player_info():
        ...
        player_number = int(input("Which player number are you? "))
    ...
    def deal_start_cards(players, deck):
    ...
    def print_field():
        ...
        for num_p, player in enumerate(players[1:]):
            ...
            for num_c, card in enumerate(player["cards"]):
                print(str(num_p) + "  " + str(player_number))
        ...
        # For the dealer
        left_offset = int((max(len(player_name_line), len(player_card_line)) - 25) / 2)
